...
def check_targets():
    ...
    assert tuple(target["size"].shape) == (2, 4, 4)
    ...
    np.testing.assert_allclose(
        decoded[0, [0, 2, 3, 4, 5, 6]],
        [1, 1.25, 1.25, 1.5, 1.0, 0.0],
        rtol=0,
        atol=1e-6,
    )
...
def check_decode():
    ...
    pred["cls"][0, 0, 0, 0] = 10
    pred["offset"][0, :, 0, 0] = torch.tensor([0.25, 0.50])
    pred["size"][0, :, 0, 0] = torch.log(torch.tensor([1.5, 3.0]))
    pred["yaw"][0, 0] = 1
    geometry = {
        "x_min": 0.0, "x_max": 2.0, "x_res": 1.0,
        "y_min": 0.0, "y_max": 2.0, "y_res": 1.0,
    ...
    invalid["size"][0, 0, 0, 0] = float("inf")
    try:
...
def check_metrics():
    ...
    first = evaluator.GroundTruth(
        ...
        x=10.0, y=0.0, length=2.0, width=2.0, yaw=0.0,
    )
    predictions = {"000001": np.array([[0, 0.99, 10, 0, 2, 2, 0]], dtype=np.float32)}
    labels = {"000001": [first]}
    accuracy = evaluator.evaluate_accuracy(predictions, labels)
    ...
    assert flat["map_moderate_percent"] == 12.0
    assert flat["mean_ap_9_percent"] == 13.0
