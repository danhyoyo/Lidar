...
def check_encoder():
    ...
    rich11 = preprocess.encode_bev(
        points,
        geometry,
        {"name": "rich11", "n_bands": 6, "density_norm": 32.0, "intensity_scale": 1.0},
    )
    assert rich11.shape == (2, 2, 11)
    np.testing.assert_array_equal(rich11[0, 0, :6], [1.0, 0.0, 1.0, 0.0, 1.0, 0.0])
    np.testing.assert_allclose(
        rich11[0, 0, 6:],
        [2.0 / 3.0, 3.2 / 9.0, 1.0, 1.6 / 3.0, np.log(4.0) / np.log(33.0)],
        rtol=0,
    ...
    np.testing.assert_array_equal(rich11[0, 1], np.zeros(11, dtype=np.float32))
    assert rich11[1, 1, 10] == 1.0
...
def check_shapes():
    ...
    config["data"]["bev_encoding"] = {"name": "rich11", "n_bands": 6}
    assert common.input_shape(config) == (1, 11, 4, 4)
...
def check_gate_scale():
    ...
    summed = Backbone(input_channels=8, scale_gated_fpn=False).eval()
    gated = Backbone(input_channels=8, scale_gated_fpn=True, gate_scale=0.5).eval()
    missing, unexpected = gated.load_state_dict(summed.state_dict(), strict=False)
    assert not unexpected
    assert set(missing) == {
        "gate_c4.weight", "gate_c4.bias", "gate_c3.weight", "gate_c3.bias"
    ...
    legacy = Backbone(input_channels=8, scale_gated_fpn=True).eval()
    probe = torch.randn(4, 8, 4, 4)
    bounded = gated._gate(probe)
    assert float(bounded.min()) >= 0.5 - 1e-6
    assert float(bounded.max()) <= 1.5 + 1e-6
    unbounded = legacy._gate(probe)
    assert float(unbounded.min()) >= 0.0 - 1e-6
    assert float(unbounded.max()) <= 2.0 + 1e-6
    assert (bounded != unbounded).any()


def check_rich11_configs():
    a2 = json.loads((ROOT / "configs/kitti/mobilebev/a2_rich8_center3d.json").read_text())
    a5 = json.loads((ROOT / "configs/kitti/mobilebev/a5_rich11_center3d.json").read_text())
    expected_a5 = json.loads(json.dumps(a2))
    expected_a5["data"]["bev_encoding"] = {
        "name": "rich11", "n_bands": 6, "density_norm": 32, "intensity_scale": 1,
    }
    expected_a5["note"] = "mobilebev_rich11_followup"
    assert a5 == expected_a5

    a4 = json.loads((ROOT / "configs/kitti/mobilebev/a4_rich8_sgfpn_center3d.json").read_text())
    a6 = json.loads((ROOT / "configs/kitti/mobilebev/a6_rich11_sgfpn_center3d.json").read_text())
    expected_a6 = json.loads(json.dumps(a4))
    expected_a6["data"]["bev_encoding"] = {
        "name": "rich11", "n_bands": 6, "density_norm": 32, "intensity_scale": 1,
    }
    expected_a6["model"]["gate_scale"] = 0.5
    expected_a6["note"] = "mobilebev_rich11_followup"
    assert a6 == expected_a6
...
CHECKS = {
    ...
    "gates": check_gates,
    "gate_scale": check_gate_scale,
    "rich11_configs": check_rich11_configs,
    "head": check_head,
...
def main():
    ...
    parser.add_argument("--gate-scale", action="store_true")
    parser.add_argument("--rich11-configs", action="store_true")
    ...
    selected = (
        ...
        ["gate_scale"] if args.gate_scale else
        ["rich11_configs"] if args.rich11_configs else
