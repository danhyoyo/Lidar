...
## Addendum 2026-09-17: follow-up chẩn đoán A4 (Pedestrian 3D AP)

Kết quả seed 42, 50 epoch trên L4: A4 (RichBEV-8 + SG-FPN) giảm **Pedestrian Moderate 3D AP từ 59.16 (A1) xuống 56.67 (−2.49)** dù mAP Moderate 3D tăng 69.09 → 69.95. Từng thành phần riêng đều nhẹ (A2 +0.13, A3 +0.43) nên đây là interaction âm, không phải lỗi single-component. Hai giả thuyết:

- **H-A (thiếu phân giải dọc):** RichBEV-8 chỉ có 3 band độ cao + z_max/z_mean làm mất cấu trúc dọc của Pedestrian (mảnh, cao) khi cần hồi quy `z_center/log(h)`.
- **H-B (gate bão hòa):** gate `2·sigmoid` trong [0, 2] có thể học triệt tiêu nhánh lateral phân giải cao (l4/l3), mất chi tiết vật nhỏ khi input là RichBEV.

Triển khai để kiểm chứng (backward-compatible, không đổi hành vi A1–A4 cũ):

1. **RichBEV-11**: `bev_encoding.name="rich11"`, `n_bands=6` (mặc định) → 6 band độ cao + `[z_max, z_mean, i_max, i_mean, density]` = 11 kênh. `encode_bev`/`input_shape` đã tham số hóa theo `n_bands`; rich8 giữ nguyên 3 band.
2. **Bounded gate** (`model.gate_scale`, opt-in): `G = 1 + gate_scale·tanh(DWConv(L+U))`, `gate_scale=0.5` → khoảng [0.5, 1.5]. Vẫn init-equivalent (G=1 khi zero-init). Config A1–A4 không set `gate_scale` nên giữ `2·sigmoid` như cũ → kết quả đã chạy không bị vô hiệu.
3. **Gate diagnostics** trong `train.py`: log mỗi epoch `gate_c4/c3_gate_mean/std/sat_low/sat_high` để xác nhận H-B (chạy trên `--gate-diag-batches` batch val, mặc định 4).

Config mới: `a5_rich11_center3d.json` (RichBEV-11 + sum-FPN, cô lập H-A so A2) và `a6_rich11_sgfpn_center3d.json` (RichBEV-11 + bounded gate, kiểm chứng A4-fix). Ngưỡng gate: **Go** nếu A6 Ped Moderate 3D AP ≥ 59.16 và mAP Moderate 3D ≥ 69.95 qua 3 seed; nếu A6 vẫn dưới thì dùng kết luận A2/A5 làm input-compression contribution và chấm dứt nhánh gate.
