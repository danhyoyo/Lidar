...
def flatten(result: Dict[str, Any]) -> Dict[str, Any]:
    row = {
        ...
        "mean_ap_9_percent": nested(result, "accuracy", "mean_ap_9_percent"),
        "map_moderate_percent": nested(result, "accuracy", "map_moderate_percent"),
    ...
    for class_name in CLASSES:
        for difficulty in DIFFICULTIES:
            row[f"{class_name.lower()}_{difficulty.lower()}_ap_r40_percent"] = nested(
                result, "accuracy", "per_class", class_name, "difficulties",
...
def markdown_table(results) -> str:
    lines = [
        "# KITTI BEV model comparison",
        "",
        "All successful models used the same frame IDs, decoder, score threshold, NMS, ROI, IoU thresholds and AP R40 implementation.",
        "",
        "| Model | Status | mAP Moderate (%) | Mean AP-9 (%) | Model mean (ms) | Model p95 (ms) | Model FPS | E2E mean (ms) | E2E p95 (ms) | E2E FPS | Peak MiB |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for result in results:
        ...
        lines.append(
            f"| {result['name']} | ok | "
            f"{fmt(nested(result, 'accuracy', 'map_moderate_percent'))} | "
            f"{fmt(nested(result, 'accuracy', 'mean_ap_9_percent'))} | "
