#!/usr/bin/env python3
"""Select a saved checkpoint by locked KITTI 3D validation metrics."""
...
def select_best(records):
    usable = [record for record in records if record["map_3d_moderate_percent"] is not None]
    if not usable:
        raise ValueError("No checkpoint produced a 3D Moderate mAP")
    return max(
        usable,
        key=lambda record: (
            record["map_3d_moderate_percent"],
            record["mean_3d_ap_9_percent"],
            -record["epoch"],
        ),
    )
...
def main(argv=None):
    ...
    if config["model"].get("box_encoding", "bev") != "center3d":
        raise ValueError("3D checkpoint selection requires model.box_encoding=center3d")
    checkpoints = []
    ...
    write_json(args.output_dir / "selection.json", {
        "criterion": "max 3D Moderate mAP, then mean 3D AP-9, then earlier epoch",
